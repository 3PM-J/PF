...
public class PacienteController {
    ...
    public ResponseEntity<Map<String, Object>> actualizar(@PathVariable Long id, @RequestBody Paciente paciente) {
        try {
            ...
            Optional<Paciente> pacienteConDni = pacienteService.buscarPorDni(paciente.getDni());
            if (pacienteConDni.isPresent() && !pacienteConDni.get().getId().equals(id)) {
                response.put("success", false);
                response.put("message", "Ya existe otro paciente con este DNI");
                return ResponseEntity.badRequest().body(response);
            }
